...
def api_register(payload: RegisterIn, request: Request):
    ...
    duplicate_voter_id = face_engine.find_duplicate(encoding, db.list_all_encodings())
    if duplicate_voter_id:
        db.log_event(
            voter_id,
            "register_duplicate_face",
            detail=f"matches existing registration {duplicate_voter_id}",
    ...
    db.create_voter(voter_id, name, email, encrypted)
...
def api_authenticate(payload: AuthenticateIn, request: Request):
    ...
    is_live, liveness_reason = face_engine.check_liveness(frames_rgb)
    if not is_live:
        db.record_failed_attempt(voter_id)
        db.log_event(voter_id, "liveness_fail", detail=liveness_reason, ip_address=ip)
